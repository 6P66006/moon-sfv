...
The files in this directory are a **pinned, read-only snapshot** of the
upstream repository at commit `1e280c3ed9ffe0ca5fdb1d97219dddc389007677`
(fetched 2026-08-04). See `SOURCE.json` for per-file SHA-256 hashes and the
import provenance. `scripts/import_httpwg_tests.py` consumes only these JSON
files to generate the MoonBit conformance data; it never reads upstream
implementation code. The upstream license is in `LICENSE.md`.
